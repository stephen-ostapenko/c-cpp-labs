...
void test_stress() {
  std::mt19937 mtw(24);
  std::vector <shared_ptr> p;

  p.push_back(make_shared(11));
  for (int it = 0; it < 1000 * 1000; it++) {
    int act = mtw() % 9;

    if (act == 0) {
      p.push_back(shared_ptr());
    } else if (act == 1) {
      int pos = mtw() % p.size();
      p.push_back(shared_ptr(p[pos]));
    } else if (act == 2) {
      int pos1 = mtw() % p.size(), pos2 = mtw() % p.size();
      p[pos1] = p[pos2];
    } else if (act == 3) {
      int pos = mtw() % p.size();
      p[pos].ptr();
